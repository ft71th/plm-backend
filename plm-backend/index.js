...
// GET /api/projects/:id/hal
app.get('/api/projects/:id/hal', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT data, version FROM project_hal WHERE project_id = $1',
      [req.params.id]
    );
    res.json(result.rows[0] || null);
  } catch (err) {
    console.error('Get HAL error:', err);
    res.status(500).json({ error: 'Failed to load HAL config' });
  }
});

// PUT /api/projects/:id/hal
app.put('/api/projects/:id/hal', authenticateToken, async (req, res) => {
  try {
    const { data, version } = req.body;
    await pool.query(
      `INSERT INTO project_hal (project_id, data, version, updated_at)
       VALUES ($1, $2, $3, NOW())
       ON CONFLICT (project_id)
       DO UPDATE SET data = $2, version = $3, updated_at = NOW()`,
      [req.params.id, JSON.stringify(data), version || '0.1']
    );
    res.json({ success: true });
  } catch (err) {
    console.error('Save HAL error:', err);
    res.status(500).json({ error: 'Failed to save HAL config' });
  }
});

// POST /api/projects/:id/hal/version — save a versioned snapshot
app.post('/api/projects/:id/hal/version', authenticateToken, async (req, res) => {
  try {
    const { data, version, changes } = req.body;
    // Save version snapshot
    const result = await pool.query(
      `INSERT INTO project_hal_versions (project_id, version, data, changes, created_by)
       VALUES ($1, $2, $3, $4, $5) RETURNING id, version, created_at`,
      [req.params.id, version, JSON.stringify(data), changes || '', req.user?.email || 'system']
    );
    // Also update current
    await pool.query(
      `INSERT INTO project_hal (project_id, data, version, updated_at)
       ...
       ON CONFLICT (project_id)
       DO UPDATE SET data = $2, version = $3, updated_at = NOW()`,
      [req.params.id, JSON.stringify(data), version]
    );
    res.json(result.rows[0]);
  } catch (err) {
    console.error('Save HAL version error:', err);
    res.status(500).json({ error: 'Failed to save HAL version' });
  }
});

// GET /api/projects/:id/hal/versions — list version history
app.get('/api/projects/:id/hal/versions', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, version, changes, created_by, created_at
       FROM project_hal_versions WHERE project_id = $1
       ORDER BY created_at DESC LIMIT 50`,
      [req.params.id]
    );
    res.json(result.rows);
  } catch (err) {
    console.error('Get HAL versions error:', err);
    res.status(500).json({ error: 'Failed to load HAL versions' });
  }
});

// GET /api/projects/:id/hal/versions/:versionId — load specific version
app.get('/api/projects/:id/hal/versions/:versionId', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM project_hal_versions WHERE id = $1 AND project_id = $2',
      [req.params.versionId, req.params.id]
    );
    res.json(result.rows[0] || null);
  } catch (err) {
    console.error('Get HAL version error:', err);
    res.status(500).json({ error: 'Failed to load HAL version' });
  }
});
...
// GET all templates
app.get('/api/doc-templates', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM doc_templates ORDER BY category, name'
    );
...
// Clone template
app.post('/api/doc-templates/:id/clone', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { name } = req.body;
    const original = await pool.query('SELECT * FROM doc_templates WHERE id = $1', [id]);
    if (original.rows.length === 0) return res.status(404).json({ error: 'Template not found' });

    const tmpl = original.rows[0];
    const result = await pool.query(
      `INSERT INTO doc_templates (name, type, category, description, version, schema, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [name || `${tmpl.name} (Copy)`, tmpl.type, tmpl.category, tmpl.description, tmpl.version, tmpl.schema, req.user.id]
    );
...
// Get single document with template schema
app.get('/api/projects/:id/documents/:docId', authenticateToken, async (req, res) => {
  try {
...
app.post('/api/projects/:id/documents', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { template_id, title, metadata } = req.body;

    // Generate doc number: DOC-XXXX
    const countResult = await pool.query('SELECT COUNT(*) FROM documents WHERE project_id = $1', [id]);
    const docNum = `DOC-${String(parseInt(countResult.rows[0].count) + 1).padStart(4, '0')}`;

    const result = await pool.query(`
      INSERT INTO documents (template_id, project_id, title, doc_number, metadata, created_by)
      VALUES ($1, $2, $3, $4, $5, $6) RETURNING *
    `, [template_id, id, title, docNum, JSON.stringify(metadata || {}), req.user.name || req.user.email]);
...
app.put('/api/projects/:id/documents/:docId', authenticateToken, async (req, res) => {
  try {
    const { id, docId } = req.params;
    const { title, status, metadata, section_data, revision_log } = req.body;
    ...
    if (section_data !== undefined)  { sets.push(`section_data = $${i++}`); vals.push(JSON.stringify(section_data)); }
    if (revision_log !== undefined)  { sets.push(`revision_log = $${i++}`); vals.push(JSON.stringify(revision_log)); }
    sets.push(`updated_at = CURRENT_TIMESTAMP`);

    vals.push(docId, id);
    const result = await pool.query(
      `UPDATE documents SET ${sets.join(', ')} WHERE id = $${i++} AND project_id = $${i} RETURNING *`,
      vals
    );
...
app.delete('/api/projects/:id/documents/:docId', authenticateToken, async (req, res) => {
  try {
    const { id, docId } = req.params;
    await pool.query('DELETE FROM documents WHERE id = $1 AND project_id = $2', [docId, id]);
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/projects/:id/documents/:docId/new-version', authenticateToken, async (req, res) => {
  try {
    const { id, docId } = req.params;
    const { changes } = req.body;

    const doc = await pool.query('SELECT * FROM documents WHERE id = $1 AND project_id = $2', [docId, id]);
    ...
    const revLog = Array.isArray(current.revision_log) ? current.revision_log : [];
    revLog.push({
      version: newVersion,
      date: new Date().toISOString().split('T')[0],
      author: req.user.name || req.user.email,
      changes: changes || 'New version',
    });

    const result = await pool.query(
      `UPDATE documents SET version = $1, revision_log = $2, status = 'draft', updated_at = CURRENT_TIMESTAMP
       WHERE id = $3 AND project_id = $4 RETURNING *`,
      [newVersion, JSON.stringify(revLog), docId, id]
    );
